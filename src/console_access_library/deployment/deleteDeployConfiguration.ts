...
/**
 * This class provide method to  delete deployment config information for a specified Config Id.
 */
export class DeleteDeployConfiguration {
    config: Config;
    api: DeployApi;
    ...
    /**
    * Schema for API to DeleteDeployConfiguration.
    ...
    */
     ...
     *    const response= await client.deployment.deleteDeployConfiguration(configId);
     */
    async deleteDeployConfiguration(configId: string) {
        Logger.info('deleteDeployConfiguration');
        let valid = true;
        try {
            const validate = ajv.compile(this.schema);
            valid = validate({ configId });
            if (!valid) {
            ...
            if (this.config.applicationId) {
                res = await this.api.deleteDeployConfiguration(configId, 'client_credentials');
            } else {
                res = await this.api.deleteDeployConfiguration(configId);
            }
            return res;
        } catch (error) {
            if (!valid) {
